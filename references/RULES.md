> Source: this file is from the upstream `beautiful-feishu-whiteboard` skill. Bundled here so this repo is self-contained. The `feishu-whiteboard-geist` skill defers to it for the hard limits of the Feishu SVG whiteboard medium and the exact render/push commands.
